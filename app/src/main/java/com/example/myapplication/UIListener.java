...
public class UIListener implements MesiboUIListener {
    ...
    public static void fetchContacts() {
        ...
        List<MesiboProfile> profiles = Mesibo.getUserProfiles();

        // Iterate through the profiles and handle each contact
        for (MesiboProfile profile : profiles) {
            String address = profile.getAddress(); // User ID
            String name = profile.getName(); // User Name

            // Example: Print user details
            System.out.println("User ID: " + address);
            System.out.println("User Name: " + name);

            // Optionally, you can display these in your app's UI, like in a ListView or RecyclerView
        }


        // Assuming Mesibo has been initialized and started

        // Get all Mesibo contacts
